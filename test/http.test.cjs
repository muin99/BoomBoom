...
async function createTestApp({ interpreter, config, quiet } = {}) {
...
const makeInterpreter = (config, client) =>
  new OpenAiInterpreterService(
    config,
    client,
    new DirectiveValidatorService(),
...
test("HTTP contract: ten reference cases, Swagger, validation and sanitized failures", async (t) => {
  ...
  const interpreter = {
    ready: () => true,
    interpret: async () =>
      structuredClone(selected.expected_output.directive_interpretation),
  };
  const { app } = await createTestApp({ interpreter, quiet: true });
  ...
  const doc = await (await fetch(base + "/docs-json")).json();
  assert.ok(doc.paths["/optimize-energy"].post.responses["200"]);
  assert.ok(
    doc.paths["/optimize-energy"].post.requestBody.content["application/json"]
      .schema.properties.operator_notes,
  );
  const post = (body) =>
  ...
  for (const c of pack.cases) {
    ...
    assert.ok(
      Math.abs(p.total_cost_bdt - c.expected_output.total_cost_bdt) < 0.01,
    );
  }
  assert.equal((await post({})).status, 400);
  const invalid = structuredClone(pack.cases[0].input);
  invalid.battery.initial_energy_kwh = 999;
  assert.equal((await post(invalid)).status, 422);
  const malformed = await fetch(base + "/optimize-energy", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: '{"bad":',
  });
  ...
  interpreter.interpret = async () => {
    throw new Error("SECRET-sentinel-stack");
  };
  const failure = await post(pack.cases[0].input);
...
test("missing key produces controlled readiness and optimization failures", async (t) => {
  ...
  const r = await fetch(base + "/optimize-energy", {
    method: "POST",
    headers: { "content-type": "application/json" },
    body: JSON.stringify(pack.cases[0].input),
  });
  assert.equal(r.status, 500);
  assert.equal((await r.json()).error, "OPENAI_KEY_MISSING");
});
...
test("LLM path retries bad structured output, caches valid output, and coalesces requests", async () => {
        ...
        return {
          ...
          output_parsed: {
            directive_interpretation:
              calls === 1
                ? []
                : structuredClone(c.expected_output.directive_interpretation),
          },
        };
      },
    },
  ...
  const interpreter = makeInterpreter(
    { ...readConfig(), apiKey: "test-only", attempts: 2 },
    fake,
  );
  const results = await Promise.all(
    Array.from({ length: 8 }, () => interpreter.interpret(c.input)),
  );
  assert.equal(calls, 2);
  assert.deepEqual(results[0], c.expected_output.directive_interpretation);
  results[0][0].explanation = "changed";
  assert.notEqual(
    (await interpreter.interpret(c.input))[0].explanation,
    "changed",
  );
  assert.equal(calls, 2);
  const changed = structuredClone(c.input);
  changed.battery.capacity_kwh++;
  ...
  for (const fake of [
    {
      responses: {
        parse: async () => ({
          status: "completed",
          output_parsed: { directive_interpretation: [] },
        }),
      },
    },
    {
    ...
    const interpreter = makeInterpreter(
      { ...readConfig(), apiKey: "test-only" },
      fake,
    );
    await assert.rejects(
      () => interpreter.interpret(pack.cases[0].input),
