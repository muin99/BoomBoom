...
const {
  requestSchema,
} = require("../dist/energy/dto/optimize-energy-request.dto");
const {
...
const validateDirectives = (raw, scenario) =>
  directiveValidator.validate(raw, scenario);
const optimize = (scenario, directives) =>
  optimizer.optimize(scenario, directives);
const replay = (scenario, raw, groundTruth) =>
  planReplay.verify(scenario, raw, groundTruth);
...
const scenario = () => ({
  ...
  operator_notes: ["Administrative notice."],
...
for (const c of pack.cases)
  test(`${c.id}: reference replay and optimal cost`, () => {
    const s = requestSchema.parse(c.input),
      d = validateDirectives(
        {
          directive_interpretation: c.expected_output.directive_interpretation,
        },
        s,
      );
    ...
    assert.ok(
      Math.abs(p.total_cost_bdt - c.expected_output.total_cost_bdt) < 0.001,
    );
  });
...
test("guardrails reject incorrect mappings, shapes, values, and ordering", () => {
  ...
  const bad = [
    ...
    [{ ...base, note_index: 1 }],
    [{ ...base, applies: false }],
    [{ ...base, directive_type: "invented" }],
    [{ ...base, structured_adjustment: null }],
    ...[[13, 12], [12, 12], [24], [-1], [1.5], []].map((hours) => [
      { ...base, structured_adjustment: { hours, factor: 0.2 } },
    ]),
    ...[-1, 1.1, Infinity, NaN, "0.2"].map((factor) => [
      { ...base, structured_adjustment: { hours: [12], factor } },
    ]),
    [
      {
        ...base,
        structured_adjustment: { hours: [12], factor: 0.2, demand_kwh: 0 },
      },
    ],
    [
      {
        ...
        structured_adjustment: { hours: [12], minimum_energy_kwh: 5 },
      },
    ],
    [{ ...noop[0], applies: true }],
    [{ ...noop[0], structured_adjustment: { hours: [1] } }],
  ];
  for (const entries of bad)
    assert.throws(() =>
      validateDirectives({ directive_interpretation: entries }, s),
    );
});
...
test("strict request rejects malformed hours, empty notes, strings, extras and nonfinite numbers", () => {
  for (const mutate of [
    ...
    (s) => (s.operator_notes = [" "]),
    (s) => (s.operator_notes = []),
    (s) => (s.operator_notes = ["a", "b", "c", "d"]),
    (s) => (s.hours[0].demand_kwh = "2"),
    (s) => (s.hours[0].solar_kwh = Infinity),
    (s) => (s.battery.capacity_kwh = -1),
    (s) => (s.extra = true),
  ]) {
    const s = scenario();
    mutate(s);
    assert.equal(requestSchema.safeParse(s).success, false);
  }
});
...
test("zero battery, zero tariffs, excess solar and fractional inputs remain valid", () => {
  for (const mode of ["zero", "free", "surplus", "fractional"]) {
    const s = scenario();
    if (mode === "zero")
      Object.keys(s.battery).forEach((k) => (s.battery[k] = 0));
    if (mode === "free") s.hours.forEach((h) => (h.tariff_bdt_per_kwh = 0));
    if (mode === "surplus") s.hours.forEach((h) => (h.solar_kwh = 100));
    if (mode === "fractional") {
      s.hours.forEach((h) => {
        h.demand_kwh = 2.123456;
        h.solar_kwh = 0.56789;
        h.tariff_bdt_per_kwh = 7.89123;
      });
      s.battery.initial_energy_kwh = 1.12345;
    }
...
test("combined hard constraints and full outages are enforced", () => {
  const s = scenario();
  s.operator_notes = ["a", "b", "c"];
  const ds = [
    {
      note_index: 0,
      applies: true,
      directive_type: "no_charge_window",
      structured_adjustment: { hours: [0, 1, 2, 23] },
      explanation: "a",
    },
    {
      note_index: 1,
    ...
    },
    {
      ...
      applies: true,
      directive_type: "minimum_battery_reserve",
      structured_adjustment: { hours: [18, 19], minimum_energy_kwh: 3 },
      explanation: "c",
    },
  ];
...
test("infeasibility is detected instead of silently relaxing a grid cap", () => {
  const s = scenario();
  s.battery.max_discharge_kwh_per_hour = 0;
  const d = [
    {
      note_index: 0,
      applies: true,
...
test("replay rejects tampered totals, state, solar and missed ground-truth directives", () => {
  const s = scenario(),
    good = optimize(s, noop);
  for (const mutate of [
    (p) => p.total_cost_bdt++,
    (p) => (p.hourly_plan[0].solar_used_kwh = 999),
    (p) => p.hourly_plan[0].battery_energy_after_kwh++,
    (p) => p.hourly_plan[0].grid_kwh++,
    (p) => (p.hourly_plan[1].hour = 0),
    (p) => (p.hourly_plan[0].battery_kwh = 999),
  ]) {
    const p = structuredClone(good);
    mutate(p);
    assert.throws(() => replay(s, p));
  }
  assert.throws(() =>
    replay(s, good, [
      {
...
test("100 unseen scenarios match an independent dynamic-programming cost oracle", () => {
  ...
  for (let trial = 0; trial < 100; trial++) {
    const s = scenario();
    s.battery.capacity_kwh = 2 + rand(7);
    s.battery.initial_energy_kwh = rand(s.battery.capacity_kwh + 1);
    s.battery.max_charge_kwh_per_hour = rand(4);
    s.battery.max_discharge_kwh_per_hour = rand(4);
    s.hours.forEach((h) => {
      h.demand_kwh = rand(8);
      h.solar_kwh = rand(10);
      h.tariff_bdt_per_kwh = rand(12);
    });
    ...
    for (const h of s.hours) {
      ...
      for (const [before, cost] of costs) {
        for (let after = 0; after <= s.battery.capacity_kwh; after++) {
          ...
          if (
            delta > s.battery.max_charge_kwh_per_hour ||
            -delta > s.battery.max_discharge_kwh_per_hour ||
            h.demand_kwh + delta < 0
          )
            continue;
          const candidate =
            cost +
            Math.max(0, h.demand_kwh + delta - h.solar_kwh) *
    ...
    const p = replay(s, optimize(s, noop));
    assert.ok(
      Math.abs(p.total_cost_bdt - costs.get(s.battery.initial_energy_kwh)) <
        1e-5,
      `trial ${trial}`,
    );
  }
