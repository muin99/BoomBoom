...
| PG §2–3 | One public service exposing both endpoints without login/VPN | **Live** at `https://bup.onukrom.xyz` (DianaHost cPanel, Node.js 20.20.2); `/health` and `/optimize-energy` verified externally with no caller auth, including Swagger at `/docs`. Render deployment kept as a fully reproducible backup, documented in README/`docs/deployment.md`, not the submitted endpoint |
| PG §3 | Endpoint continuously reachable; tests from outside development environment | `BASE_URL=https://bup.onukrom.xyz npm run test:judge` passed 46/46 externally (10 public cases, 18 edge cases, guardrails, concurrency); tested from this development machine only — a second network/device check is still recommended. Shared cPanel/Passenger hosting can recycle an idle application; see the note in `docs/submission.md` |
