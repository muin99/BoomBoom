...
const replay = (scenario, raw, groundTruth, tolerance) =>
  planReplay.verify(scenario, raw, groundTruth, tolerance);
function equivalent(a, b, path = "directive") {
  if (typeof b === "number") {
    assert.equal(typeof a, "number", path);
    assert.ok(Math.abs(a - b) <= 0.01, path);
    return;
  }
  ...
  if (Array.isArray(b)) {
    ...
    b.forEach((v, i) => equivalent(a[i], v, `${path}[${i}]`));
    return;
  }
  assert.deepEqual(
    Object.keys(a)
  ...
  for (const [k, v] of Object.entries(b)) {
    if (k !== "explanation") equivalent(a[k], v, `${path}.${k}`);
  }
}
async function verifyCase(base, c) {
  const started = performance.now();
  const response = await fetch(base + "/optimize-energy", {
    method: "POST",
  ...
  if (response.status !== 200) {
    const code = /^[A-Z_]+$/.test(p.error) ? p.error : "REQUEST_FAILED";
    throw new Error(`${c.id}: HTTP ${response.status} ${code}`);
  }
  equivalent(
    p.directive_interpretation,
  ...
  assert.ok(
    ...
    `${c.id}: optimal cost mismatch`,
  );
  return {
    id: c.id,
    status: "passed",
    latency_ms: Math.round(performance.now() - started),
    cost_bdt: p.total_cost_bdt,
  };
}
